...
function questionMarkup(question){
 const titleId=`v15-question-title-${question.id}`;
 const labelledBody=question.body
  .replace(/<div class="v15-options" role="radiogroup">/g,`<div class="v15-options" role="radiogroup" aria-labelledby="${titleId}">`)
  .replace(/<div class="v15-options" data-choice-group=/g,`<div class="v15-options" role="group" aria-labelledby="${titleId}" data-choice-group=`)
  .replace(/<div class="v15-rating" role="radiogroup" aria-label="[^"]+">/g,`<div class="v15-rating" role="radiogroup" aria-labelledby="${titleId}">`);
 return `<article class="v15-question" id="v15-question-${question.id}" data-question-id="${question.id}" data-question-title="${escapeHtml(question.title)}" data-required="${REQUIRED_IDS.has(String(question.id))?'true':'false'}">
  <div class="v15-question-head"><span class="qnum">${question.id}</span><div><h2 class="qtitle" id="${titleId}">${escapeHtml(question.title)}</h2>${question.tag?`<span class="qtag">${escapeHtml(question.tag)}</span>`:''}</div></div>${audioRecorder(question.id)}
  <div class="v15-answer">${labelledBody}</div>
...
function formMarkup(){
 return `<div class="v15-app" id="v15ShortForm">
  <header class="v15-toolbar" id="v15Toolbar">
   ...
   <div class="v15-toolbar-progress" aria-label="התקדמות מילוי"><div><span id="v15ProgressText">0 מתוך 16</span><span id="v15SaveState">נשמר אוטומטית</span></div><div class="v15-progress-track"><span id="v15ProgressBar"></span></div></div>
...
function requestRecordingConsent(){
 ...
 if(!dialog){panel?.scrollIntoView({behavior:motionBehavior(),block:'center'});panel?.classList.add('is-highlighted');setTimeout(()=>panel?.classList.remove('is-highlighted'),1900);$('#v15PrivacyAck')?.focus();return Promise.resolve(false)}
 ...
 return new Promise(resolve=>{
  const focusables=()=>$$('button:not([disabled]),[href],input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])',dialog).filter(element=>!element.hidden&&element.offsetParent!==null);
  const finish=approved=>{approve?.removeEventListener('click',onApprove);cancel?.removeEventListener('click',onCancel);dialog.removeEventListener('click',onBackdrop);document.removeEventListener('keydown',onKey,true);dialog.hidden=true;document.body.classList.remove('v15-modal-open');if(approved){const checkbox=$('#v15PrivacyAck');if(checkbox&&!checkbox.checked){checkbox.checked=true;checkbox.dispatchEvent(new Event('change',{bubbles:true}))}}previousFocus?.focus?.();resolve(approved)};
  const onApprove=()=>finish(true),onCancel=()=>finish(false),onBackdrop=event=>{if(event.target===dialog)finish(false)},onKey=event=>{if(event.key==='Escape'){event.preventDefault();finish(false);return}if(event.key!=='Tab')return;const controls=focusables();if(!controls.length){event.preventDefault();return}const first=controls[0],last=controls[controls.length-1],active=document.activeElement;if(!dialog.contains(active)){event.preventDefault();(event.shiftKey?last:first).focus()}else if(event.shiftKey&&active===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&active===last){event.preventDefault();first.focus()}};
  approve?.addEventListener('click',onApprove);cancel?.addEventListener('click',onCancel);dialog.addEventListener('click',onBackdrop);document.addEventListener('keydown',onKey,true);cancel?.focus();
 });
}
...
function focusMissing(item){
 if(!item)return;
 showValidationFor(item);
 item.container?.classList.add('v15-missing-focus');
 item.container?.scrollIntoView({behavior:motionBehavior(),block:'center'});
 setTimeout(()=>{item.target?.focus?.();item.container?.classList.remove('v15-missing-focus')},450);
}

function clearValidationStyles(){
 $$('.v15-question').forEach(card=>{card.classList.remove('has-error');const error=$('.v15-error',card);if(error)error.textContent=''});
 $$('.v15-identity label').forEach(label=>label.classList.remove('has-error'));
 $('#v15ConsentPanel')?.classList.remove('has-error');
}

function showValidationFor(item){
 validationShown=true;validationTarget=item?.target||null;clearValidationStyles();
 if(!item?.container)return;
 if(item.kind==='question'){
  const recorded=item.container.dataset.hasRecording==='true';item.container.classList.add('has-error');
  const error=$('.v15-error',item.container);if(error)error.textContent=recorded?'צריך לבדוק ולאשר את התמלול לפני השליחה.':'צריך להשלים את השאלה לפני השליחה.';
 }else if(item.kind==='identity')item.container.classList.add('has-error');
 else if(item.kind==='consent')item.container.classList.add('has-error');
}
...
function updateProgress({showErrors=false}={}){
 ...
 const cards=$$('.v15-question'),required=cards.filter(card=>REQUIRED_IDS.has(card.dataset.questionId)),answeredRequired=required.filter(isAnswered);
 cards.forEach(card=>card.classList.toggle('is-answered',isAnswered(card)));
 ...
 const answeredBase=answeredRequired.length+Number(nameReady)+Number(roleReady),baseTotal=required.length+2,progressText=$('#v15ProgressText'),progressBar=$('#v15ProgressBar'),consentNote=hasRecordings&&!privacyReady?' · נדרש אישור הקלטות':'';
 if(progressText)progressText.textContent=`${answeredBase} מתוך ${baseTotal}${consentNote}`;
 if(progressBar){const progressRatio=hasRecordings&&!privacyReady?Math.min(answeredBase/baseTotal,.94):answeredBase/baseTotal;progressBar.style.width=`${Math.round(progressRatio*100)}%`}
 if(validationShown){
  const activeItem=missingItems.find(item=>item.target===validationTarget);
  if(activeItem)showValidationFor(activeItem);else if(validationTarget){validationTarget=null;clearValidationStyles()}else if(showErrors)showValidationFor(missingItems[0]);
 }
...
function bind(){
 ...
 document.addEventListener('click',event=>{
  ...
  const submit=$('#v9FinalSubmit');submit?.scrollIntoView({behavior:motionBehavior(),block:'center'});$('#v9Send')?.focus();
 },true);
...
function revealReady(){
 if(window.__vivacePatchLoadFailed){window.__vivaceBootError?.();return}
