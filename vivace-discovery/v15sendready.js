...
async function renderRecording(questionId,source=undefined){
 ...
 if(!blob){card.dataset.hasRecording='false';playback.hidden=true;playback.replaceChildren();button.classList.remove('recording','has-recording');label.textContent='הקלט תשובה';timer.textContent='00:00';status.textContent=$('#v15PrivacyAck')?.checked?'אפשר לענות בקול במקום להקליד':'יש לאשר שימוש בהקלטה';updateProgress();return}
...
async function startRecording(questionId,box){
 const qid=String(questionId),button=$('.record-button',box),label=$('.record-label',button),timer=$('.record-timer',button),status=$('.record-status',box);
 if(!$('#v15PrivacyAck')?.checked){showToast('כדי להקליט, צריך לאשר תחילה את שימוש ההקלטה לתמלול.');$('#v15PrivacyAck')?.focus();return}
 ...
 try{
  ...
  recorder.onstop=()=>{const blob=new Blob(activeChunks,{type:recorder.mimeType||mimeType||'audio/webm'});activeStream?.getTracks().forEach(track=>track.stop());activeStream=null;activeRecorder=null;activeQuestionId='';activeChunks=[];clearInterval(timerHandle);timerHandle=null;button.classList.remove('recording');if(blob.size<1){status.textContent='לא נקלט אודיו. נסה שוב.';label.textContent='הקלט תשובה';showToast('לא נקלט אודיו. נסה להקליט שוב.');return}void putRecording(qid,blob).then(()=>renderRecording(qid,blob)).then(()=>showToast(`ההקלטה של שאלה ${qid} נשמרה`)).catch(error=>{console.error('Vivace recording save failed',error);void renderRecording(qid,blob);showToast('ההקלטה נשמרה זמנית במכשיר')})};
  recorder.onerror=event=>{console.error('Vivace MediaRecorder error',event);status.textContent='ההקלטה נעצרה בגלל תקלה';stopRecording()};recorder.start(250);button.classList.add('recording');button.classList.remove('has-recording');label.textContent='עצור הקלטה';status.textContent='מקליט עכשיו… לחץ שוב לעצירה';timerStartedAt=Date.now();timer.textContent='00:00';timerHandle=setInterval(()=>{timer.textContent=formatTime(Date.now()-timerStartedAt);if(Date.now()-timerStartedAt>=MAX_RECORDING_MS){stopRecording();showToast('ההקלטה נעצרה אוטומטית אחרי 5 דקות')}},250);
...
function syncAudioConsent(){
 ...
 $$('.audio-recorder').forEach(box=>{const button=$('.record-button',box),status=$('.record-status',box),recorded=box.closest('.v15-question')?.dataset.hasRecording==='true';if(button){button.disabled=false;button.classList.toggle('is-locked',!approved);button.setAttribute('aria-disabled',approved?'false':'true')}if(status&&!button?.classList.contains('recording'))status.textContent=approved?(recorded?'ההקלטה נשמרה וניתן להאזין לה':'אפשר לענות בקול במקום להקליד'):(recorded?'ההקלטה שמורה; יש לאשר כדי לתמלל ולשלוח':'יש לאשר שימוש בהקלטה')});
}
