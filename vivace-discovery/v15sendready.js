 ...
 return `<div class="v15-app" id="v15ShortForm">
  ...
  <div class="v15-consent-dialog" id="v15ConsentDialog" role="dialog" aria-modal="true" aria-labelledby="v15DialogTitle" hidden>
   <div class="v15-consent-sheet">
    <span class="v15-dialog-icon" aria-hidden="true">🎙️</span>
...
function syncAudioConsent(){
 ...
 $$('.audio-recorder').forEach(box=>{const button=$('.record-button',box),status=$('.record-status',box),recorded=box.closest('.v15-question')?.dataset.hasRecording==='true';if(button){button.disabled=false;button.classList.toggle('is-locked',!approved);button.setAttribute('aria-disabled',approved?'false':'true');button.setAttribute('aria-label',approved?`הקלטת תשובה לשאלה ${box.dataset.recorderFor}`:`הקלטת תשובה לשאלה ${box.dataset.recorderFor} — ייפתח אישור לפני ההקלטה`)}if(status&&!button?.classList.contains('recording'))status.textContent=approved?(recorded?'ההקלטה נשמרה וניתן להאזין לה':'אפשר לענות בקול במקום להקליד'):(recorded?'ההקלטה שמורה; לחץ כדי לאשר ולשלוח':'לחץ כדי לאשר ולהקליט')});
}
